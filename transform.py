...
def add_companies(g, base_uri, csv_data, namespace):
    ...
    ben = Namespace("http://benettondata.it/graph/")
    ...
    for index, row in csv_data.iterrows():
        ...
        process_type_mapping = {
        'MANUFACTURING': ben.Manufacturing,
        'OTHER_PROCESSING': ben.OtherProcessing,
        'PRINTING': ben.Printing,
        'SPINNING': ben.Spinning,
        'WEAVING': ben.Weaving,
        'WET_PROCESS': ben.WetProcess
        }

        product_type_mapping = {
        'APPAREL': ben.Apparel,
        'SHOES': ben.Shoes,
        'ACCESSORIES': ben.Accessories
        }

        # Adding process types
        for col, rdf_type in process_type_mapping.items():
            if row[col]:
                g.add((subject_uri, ben.hasProcess, rdf_type))

        # Adding product types
        for col, rdf_type in product_type_mapping.items():
            if row[col]:
                g.add((subject_uri, ben.hasProductType, rdf_type))

        # Adding certifications
        if row['hasCertifications']:
            for cert in row['CERTIFICATIONS'].split(';'):
                sanitized_cert = cert.strip().replace(' ', '_').replace('.', '').replace(',', '')
                cert_uri = URIRef(base_uri + sanitized_cert)
                g.add((subject_uri, ben.hasCertificate, cert_uri))
...
# Example usage
main('data/csv/BenettonData.csv', 'data/benettonsuppliers.ttl')
